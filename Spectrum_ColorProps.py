...
import colour # conda install colour-science # type: ignore
import numpy as np
import pandas as pd
...
from cache import load_colour_solid_from_csv # type: ignore
from analysis import max_chroma_at_hue, Lstar_to_Yrel # type: ignore
# %%
def hue_maxchroma_properties(target_hue: float) -> dict:
    ...
    path = "C:/Users/SchwarzN/OneDrive - Université de Fribourg/Institution/P1_BraggSpericalPigments/Simulation/Rosch_MacAdam_OptimalColours/rosch_macadam_max_chroma_per_hue_1deg.csv"
    df_max_chroma = pd.read_csv(path)
...
def colour_spectrum(wavelength_nm: float, intensity: float = 1.0) -> colour.SpectralDistribution:
    ...
    intensity: Intensity value shape = [N]
...
def compute_color_properties(
    ...
    input_spec: Intensity values shape = [N]
